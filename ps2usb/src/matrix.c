...
extern volatile uint8_t ps2_error_data ;
extern uint8_t	ps2_err_state ;
extern uint16_t timeSamlpes[12];
...
struct dumpData {
	uint8_t errData;
	uint8_t errState;
	uint16_t timeSamples[10];
	uint8_t lastTenKeys[HISTORY_LENGHT];
};
struct dumpData dumpDataTab[10];

uint8_t dumpCounter = 0;

void printDump(void){
	uint8_t i;
	DBG2(0xaa,&dumpCounter,1);
	DBG2(0xaa,&dumpCounter,1);
	for (i =0 ; i < dumpCounter ; i++){
		DBG2(0xae,&i,1);
		DBG2(0xab,&dumpDataTab[i].errState,1);
		DBG2(0xac,&dumpDataTab[i].errData,1);
		DBG2(0xad,dumpDataTab[i].timeSamples,20);
		DBG2(0xaf,dumpDataTab[i].lastTenKeys,HISTORY_LENGHT);
	}
	dumpCounter = 0; // print out all debug data. Rewrite new bugs from start of table
...
{
	uint8_t i;
	static uint8_t lastTenKeys[HISTORY_LENGHT] = {0};
	static uint8_t lastTenKeysCounter = 0;
	...
	if (ps2_error){
		//trunOffMouseRx();
		DBG2(0xab,&ps2_error,1);
		DBG2(0xac,&ps2_error_data,1);
		//DBG2(0xad,timeSamlpes,24);
		//DBG2(0xad,&ps2_err_state,1);
		//DBG2(0xae,&value,1);
		
		//save do dump in sleep state where we have no usb communication
		dumpDataTab[dumpCounter].errData = ps2_error_data;
		dumpDataTab[dumpCounter].errState = ps2_error;
		...
		for (i = 0; i < HISTORY_LENGHT; i++){
			//dumpDataTab[dumpCounter].timeSamples[i] =  timeSamlpes[i+2];
			dumpDataTab[dumpCounter].lastTenKeys[i] = lastTenKeys[(lastTenKeysCounter-i+HISTORY_LENGHT - 1 )%HISTORY_LENGHT];
		}
		dumpCounter++;
		// overflow check
		if (dumpCounter > 9 )
		...
		//DBG2(0x0b,&code,1);
		// write history of pressed keys
		lastTenKeys[lastTenKeysCounter] = code;
		lastTenKeysCounter = ++lastTenKeysCounter % HISTORY_LENGHT;
	}
